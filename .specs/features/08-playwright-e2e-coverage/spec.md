...
2. WHEN screenshots are generated during the initial test execution run THEN the agent SHALL inspect each captured image to verify form field alignment, button paddings, typography scale, and Glassmorphism backdrop-blur rendering per `DESIGN.md`.
3. IF any visual defect (misaligned inputs, broken contrast, unreadable text, or overflowing containers) is identified during visual inspection THEN the system SHALL log the defect with remediation steps.
