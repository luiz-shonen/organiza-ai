...
## 5. Gerenciamento de Sessão e Acesso
- **Admin**: Usa Firebase Authentication padrão. É gerido pelo `AuthService` e validado com um `AuthGuard` robusto que lida com o tempo de latência na inicialização do app.
...
- Todas as operações devem passar pela validação em `firestore.rules`.
