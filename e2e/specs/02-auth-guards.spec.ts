...
test.describe('Authentication Guards and Form Validation', () => {
  ...
  test('should display inline validation errors when submitting invalid email and password', async ({ page, loginPage }) => {
    ...
    const emailError = page.locator('mat-error').filter({ hasText: /Digite um e-mail válido|E-mail é obrigatório/i });
    const passwordError = page.locator('mat-error').filter({ hasText: /Mínimo 6 caracteres|Senha é obrigatória/i });
