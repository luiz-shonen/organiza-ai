...
**Decision:** Tailwind CSS was completely removed from the project. Styling is done exclusively via SCSS with BEM methodology + Angular Material components customized via CSS Custom Properties (--org-_, --mat-sys-_).  
...
**Date:** 2026-09-04  
**Decision:** (1) Implemented `scripts/create-pr.mjs` (`npm run pr:create`) to push feature branches and open Pull Requests via GitHub CLI (`gh`) using an explicit personal token (`GH_TOKEN` from `.env.local`), avoiding user session conflicts on corporate machines without storing secrets in git; (2) Added post-CI AI Code Review step (`gemini-review` in `.github/workflows/ci.yml`) executed via Google Gemini Flash models with graceful fallback (`gemini-3.8-flash` down to `gemini-3.5-flash`), chained after `quality` (linters, contracts, build) and `e2e` (Playwright) gates pass; (3) The AI review engine (`scripts/gemini-pr-review.mjs`) automatically discovers matching feature specifications in `.specs/features/` and cross-references PR diffs against `spec.md` acceptance criteria, domain workflows from `CONTEXT.md`, and architecture constraints from `AGENTS.md` and `DESIGN.md`; (4) Automatic review runs once on initial PR opening (`github.event.action == 'opened'`), skipping runner execution on subsequent pushes (`synchronize`) to save 100% of runner minutes and AI tokens; subsequent updates require on-demand `/review` or `/gemini review` comments via `.github/workflows/gemini-review-comment.yml`; (5) Findings are categorized using industry standard priority taxonomy: 🔴 P1 (Blocker/Architectural), 🟡 P2 (Major/Technical Debt), 🔵 P3 (Minor/Cosmetic); (6) Out-of-scope opportunities are structured as comprehensive TLC Spec-Driven issue drafts in EARS notation with ready-to-use Antigravity agent prompts, and automatically opened and linked as GitHub issues via REST API (with duplicate check); (7) All repository scripts (`scripts/**/*.mjs`, `*.mjs`) have automated unit tests (`npm run test:scripts`) and are fully integrated into ESLint, Stylelint, and Prettier under the unified `npm run quality` gate.  
**Rationale:** Eliminates manual architectural policing, prevents comment spam on repeated commits, provides clear actionable priority tiers, ensures strict spec traceability and EARS acceptance criteria compliance before merge, prevents token and runner waste on broken builds/tests or repeated pushes, automates issue generation for future development cycles, and provides frictionless multi-account GitHub workflows on developer workstations.  
**Status:** In force.
