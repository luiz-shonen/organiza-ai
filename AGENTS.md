...
- **CI/CD, Multi-Account PRs & AI Code Review (AD-043, AD-045)**:
  ...
  - **Gemini AI Review Gatekeeper (`scripts/gemini-pr-review.mjs`)**: Automated architectural, domain, and spec compliance review using Gemini Flash with model fallback (`gemini-3.8-flash` down to `gemini-3.5-flash`), cross-referencing `AGENTS.md`, `DESIGN.md`, `CONTEXT.md`, and matching `.specs/features/` acceptance criteria. Runs only after `quality` and `e2e` succeed. Runs automatically once on PR creation; subsequent updates require on-demand `/review` comments (with a single non-duplicated guidance notice). Categorizes findings into P1/P2/P3 priorities and formats out-of-scope opportunities into TLC Spec-Driven issue drafts (EARS).
